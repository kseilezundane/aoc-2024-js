...
async function calculateExtendedCalibrationResult() {
    ...
    for await (const line of readlineInterface) {
        ...
        const lineResult = numbers.shift();
        ...
        for (const permutation of permutationsCache[operatorsCount]) {
            ...
            for (let i = 1; i < numbers.length; i++) {
                if (permutation[i - 1] === "*") {
                    ...
                    // abusing JS hack to concatenate numbers as string
...
await calculateExtendedCalibrationResult();
